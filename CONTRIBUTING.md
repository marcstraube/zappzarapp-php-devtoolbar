# Contributing to zappzarapp/devtoolbar
...
```bash
git clone git@github.com:marcstraube/zappzarapp-php-devtoolbar.git
cd zappzarapp-php-devtoolbar
make install
make hooks
```
...
Deptrac currently runs against a single-layer stub (the whole
`Zappzarapp\DevToolbar\*` namespace) so it is wired into CI from day one. The
intended layering — to be enforced with a ruleset once the boundaries settle —
follows the dependency direction:

```text
Config → Guard → DataCollectors → Analyzers → Renderers → entry points
                                                           (DevToolbar, Middleware)
```

Run `make deptrac` to verify architecture compliance. See `deptrac.yaml` for the
current layer definition and the TODO tracking the full split.
